...
async function getData() {
  ...
  try {
    ...
    const [renunganRes, agendaRes, sliderRes] = await Promise.all([
      fetch(`${baseUrl}/api/renungan?limit=3`, { cache: 'no-store' }),
      fetch(`${baseUrl}/api/agenda?limit=3`, { cache: 'no-store' }),
    ...
    <div className="min-h-screen bg-[var(--background)] font-sans text-white">
      ...
      <section className="py-24 px-6 relative overflow-hidden bg-[var(--background)]">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-8">
          <div className="inline-block px-4 py-1.5 bg-blue-500/10 text-[var(--primary-light)] text-sm font-semibold tracking-wide uppercase rounded-full border border-blue-500/20">
            Selamat Datang
          </div>
      ...
      {/* Schedules - Modern Card Layout with Dark Theme */}
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={idx} className="bg-[var(--card-bg)] p-8 rounded-2xl border border-[var(--border-color)] hover:border-[var(--primary)] transition-all group text-center hover:scale-105">
      ...
      {/* Latest Updates Grid - Dark Theme */}
      <section className="py-24 px-6 bg-[var(--background)]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {/* Renungan Cards */}
            {renungan.map((item) => (
              <Link key={item.renungan_id} href={`/renungan/${item.renungan_id}`} className="group bg-[var(--card-bg)] rounded-2xl overflow-hidden border border-[var(--border-color)] hover:border-[var(--primary)] transition-all hover:scale-105">
      ...
      {/* Visi & Misi Section - Dark Background */}
